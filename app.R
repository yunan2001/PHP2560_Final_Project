...
#Retirement Plan Function
retire_plan <- function(D, x, age, current_year, retire_year, i, p) {
  #create a data frame that stores year, age and profile value
  col_names <- c("year","age", "profile_value")
  profile_df <- data.frame(matrix(ncol=length(col_names), nrow=0))
  colnames(profile_df) <- col_names
  
  #calculate the year between retirement and current 
  n <- retire_year-current_year
  
  #create a data frame that stores year, age and profile value before retirment
  ...
  #create a data frame that stores year, age and profile value after retirment
  ...
  #create a vector to store profile values
  value_list <- c()
  
  #loop from current year to retirement year
  for (j in 0:n){
    #calculate each year's profile value and add to value_list
    current_value <- D(1+i)^n+x*(((1+i)^n-1)/i)
    value_list <- c(value_list, current_value)
  }
  #combine before_retire data frame to the profile_df data frame
